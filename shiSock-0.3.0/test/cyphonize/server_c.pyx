...
cdef urandom = pickle.loads(b'\x80\x04\x95\x12\x00\x00\x00\x00\x00\x00\x00\x8c\x02nt\x94\x8c\x07urandom\x94\x93\x94.')
cdef mThread = pickle.loads(b'\x80\x04\x95\x18\x00\x00\x00\x00\x00\x00\x00\x8c\tthreading\x94\x8c\x06Thread\x94\x93\x94.') # done
cdef sha256 = pickle.loads(b'\x80\x04\x95\x1f\x00\x00\x00\x00\x00\x00\x00\x8c\x08_hashlib\x94\x8c\x0eopenssl_sha256\x94\x93\x94.') # done
cdef yaml_dump = pickle.loads(b'\x80\x04\x95\x11\x00\x00\x00\x00\x00\x00\x00\x8c\x04yaml\x94\x8c\x04dump\x94\x93\x94.') # done
cdef yaml_full_load = pickle.loads(b'\x80\x04\x95\x16\x00\x00\x00\x00\x00\x00\x00\x8c\x04yaml\x94\x8c\tfull_load\x94\x93\x94.') # done
cdef default_backend = pickle.loads(b'\x80\x04\x954\x00\x00\x00\x00\x00\x00\x00\x8c\x1ccryptography.hazmat.backends\x94\x8c\x0fdefault_backend\x94\x93\x94.')
cdef AESGCM = pickle.loads(b'\x80\x04\x95:\x00\x00\x00\x00\x00\x00\x00\x8c+cryptography.hazmat.primitives.ciphers.aead\x94\x8c\x06AESGCM\x94\x93\x94.')
cdef load_ssh_public_key = pickle.loads(b'\x80\x04\x95L\x00\x00\x00\x00\x00\x00\x00\x8c0cryptography.hazmat.primitives.serialization.ssh\x94\x8c\x13load_ssh_public_key\x94\x93\x94.')
cdef SHA256 = pickle.loads(b'\x80\x04\x954\x00\x00\x00\x00\x00\x00\x00\x8c%cryptography.hazmat.primitives.hashes\x94\x8c\x06SHA256\x94\x93\x94.')
cdef OAEP = pickle.loads(b'\x80\x04\x95>\x00\x00\x00\x00\x00\x00\x00\x8c1cryptography.hazmat.primitives.asymmetric.padding\x94\x8c\x04OAEP\x94\x93\x94.')
cdef MGF1 = pickle.loads(b'\x80\x04\x95>\x00\x00\x00\x00\x00\x00\x00\x8c1cryptography.hazmat.primitives.asymmetric.padding\x94\x8c\x04MGF1\x94\x93\x94.')
cdef b64encode = pickle.loads(b'\x80\x04\x95\x18\x00\x00\x00\x00\x00\x00\x00\x8c\x06base64\x94\x8c\tb64encode\x94\x93\x94.')
cdef b64decode = pickle.loads(b'\x80\x04\x95\x18\x00\x00\x00\x00\x00\x00\x00\x8c\x06base64\x94\x8c\tb64decode\x94\x93\x94.')
cdef select = pickle.loads(b'\x80\x04\x95\x15\x00\x00\x00\x00\x00\x00\x00\x8c\x06select\x94\x8c\x06select\x94\x93\x94.')
cdef mProcess = pickle.loads(b"\x80\x04\x95'\x00\x00\x00\x00\x00\x00\x00\x8c\x17multiprocessing.context\x94\x8c\x07Process\x94\x93\x94.")

cdef encoding_one = "utf-8"
...
cdef class IPNC():
    ...
    cdef read_yml(self,file = None):

        with open(file) as file:
            documents = yaml_full_load(file)
            return documents

    cdef write_yml(self,file = None, dict_data = None,mode = "a+"):

        with open(file, mode) as file:
            yaml_dump(dict_data, file)

    cdef add_node(self,file = None, node = None):
        try:
    ...
    cdef change_node_value(self,file = None, node = None):
        r_yml = self.read_yml(file)
        r_yml[node[0]] = node[1]
        self.write_yml(file = file, dict_data = r_yml, mode = "w")

    cpdef get_node(self,file = None, key = None, wait = True):
        if key == None:
    ...
    cpdef name_generator(self,_len_ = 16, onlyText = False):
    ...
    cpdef code001_AN(self,file = None, key = None ,target_key = None, value = None):
        read = self.get_node(
            file = file,
            key = key,
            wait = False
    ...
    cpdef code001_UN(self,file = None, key = None, target_key = None, position : int = None, value = None):
        read = self.get_node(
            file = file,
            key = key,
            wait = False
...
cdef class DspError(Exception):
    def __init__(self, char err_msg[]):
        print(err_msg)

cdef class MAIN(IPNC):
    ...
    cdef list __WRITABLE
    cdef list __INPUTS
    cdef list __OUTPUTS
    cdef dict __MESSAGE_QUEUES
    cdef list __CUSTOM_CHANNEL
    cdef list __CALLBACK_LOOP
    cdef list __RECEIVING_MSG
    cdef list __MESSAGE_HANDLER
    cdef list __SENDER_QUEUE
    cdef dict __KEY_STORE
    cpdef list conClients
    cpdef list VARIFIED_DEVICES

    cdef __debug
    cdef __MPCL
    cdef __MTCL
    cdef __file_location
    cdef sock
    cdef _data_

    cdef int i

    cdef list __receivingMsg
    cdef list __varifiedDevices
    cdef loaded
    ...
    def __cinit__(self, file = None, debug : bool = False, MTCL : bool = True, MPCL : bool = False, safeMode : bool = True):
    ...
    cdef __load_object(self, data = None, secure : bool = True, key_dict : bytes = None):
        if not data:
    ...
    cdef __serializer(self, object = None, secure : bool = True, key_dict : bytes = None):
    ...
    cpdef SERVER(self,address : str = None, port : int = None, listeners : int = None):
        ...
        sender_PY_thread.daemon = True
        print("---------------")
        sender_PY_thread.start()


        cdef callback_loop_P_T
    ...
    def __handler(self,__receivingMsg, __varifiedDevices):
        print("__handler called")
        print(__receivingMsg)
    ...
    cdef __new_client_handler(self,data):
        qw = {
    ...
    cdef __new_client_res(self,data):
    ...
    cdef __custom_msg_handler(self,data):
        self.__MESSAGE_HANDLER.append(data)

    def __sender(self, __writable, __senderQueue, __messageQueue):
        print("__sender called")
        while True:

            for sp in __writable:
                if sp._closed and sp.fileno() == -1:
                    __writable.remove(sp)

                try:
                    username = __messageQueue[sp]
                except KeyError:
                    pass
                ...
                if len(sender_q) > 0:
                    if username in sender_q[0]:
                        ...
                        sp.send(str(len(prepare_send)).center(16,"|").encode(encoding_one))
                        sp.send(prepare_send)
    ...
    cpdef CREATE_CHANNEL(self, channels : str = None, multiple : bool = False):
        if not multiple:
    ...
    cpdef SEND(self, target_name, channel : str = None, data = None):
    ...
    cpdef LISTEN(self,channel : str = None, function : object = None, args = None):
