...
class MAIN():
    ...
    def __init__(self,client_name : str = None, debug : bool = False,  MPCL : bool = False, MTCL : bool = True):

        self.__debug = debug

        if not client_name:
            raise TypeError("MAIN() missing 1 required positional argument: 'client_name'")
        else:
            if type(client_name) != type("str"):
                raise TypeError("'client_name' should be of type string")
            else:
